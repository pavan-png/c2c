...
const ServicesSection = () => {
  return (
    <AnimatedSection
      id="services"
      className="overflow-hidden bg-[linear-gradient(90deg,#10233a_0%,#13263f_24%,#f4f4f4_24%,#f4f4f4_100%)] py-16 md:py-24"
    >
      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 md:px-6 lg:grid-cols-[0.9fr_1.65fr] lg:items-start">
        <div className="relative overflow-hidden rounded-[2rem] bg-[linear-gradient(180deg,#122238_0%,#0f1d31_100%)] px-7 py-10 text-white shadow-[0_24px_70px_rgba(2,6,23,0.28)] md:px-10 lg:min-h-[760px]">
          ...
          <motion.div
            ...
            className="inline-flex items-center gap-3 rounded-full bg-white/8 px-4 py-2 text-sm font-semibold text-orange-400 backdrop-blur-xl"
          >
          ...
          <motion.h2
            ...
            className="mt-8 max-w-md text-4xl font-bold leading-tight tracking-[-0.045em] md:text-[3.15rem]"
          >
          ...
          >
            {serviceHighlights.map((item) => (
              <li key={item} className="flex items-start gap-4 text-[1.05rem] leading-8 text-white/92 md:text-[1.15rem]">
                <span className="mt-2 h-3.5 w-3.5 shrink-0 rounded-full bg-white shadow-[0_0_0_6px_rgba(255,255,255,0.08)]" />
                <span>{item}</span>
              </li>
            ))}
          </motion.ul>
          ...
          {serviceCards.map((card, index) => {
            ...
            return (
              <motion.article
                ...
                className={`group relative overflow-hidden bg-white shadow-[0_18px_55px_rgba(15,23,42,0.08)] ${card.shape} ${card.size}`}
              >
                <img
                  src={card.image}
                  alt={card.title}
                  ...
                  loading="lazy"
                />
                <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(10,18,33,0.04),rgba(10,18,33,0.12)_36%,rgba(10,18,33,0.72)_100%)]" />
                <div className="absolute inset-x-0 bottom-0 p-5 md:p-6">
                  <div className="p-1 transition duration-200 group-hover:-translate-y-1">
                    <h3 className="text-lg font-bold tracking-[-0.035em] text-white drop-shadow-md md:text-[1.35rem]">{card.title}</h3>
